            ...
            if (fork() == 0) {
                ...
                while (recv(client_fd, &msg, sizeof(MsgBuf), 0) > 0) {
                    ...
                    if (!row) {
                        ...
                        float bal = msg.money + atof(row[1]);
                   ...
                   sprintf(sql2, "SELECT pwd,balance FROM accounts WHERE id=%d"  ,msg.to_account);
                   mysql_query(conn, sql1);
                    ...
                    if (!row1) {
                        ...
                        float bal = msg.money - atof(row1[1]);
                        float balTobalance = msg.money + atof(row2[1]);
                        ...
                        mysql_query(conn, sql1);
                             ...
                             sprintf(sql2, "UPDATE accounts SET balance=%.2f WHERE id=%d",
                                balTobalance, msg.to_account);
                        mysql_query(conn, sql2);

                        sprintf(msg.result, "转账成功！给对方存入 %.2f 元，对方当前余额 %.2f 元",
                                msg.money, balTobalance);
                    }
                    mysql_free_result(res);
                    }
